 ...
 *        Revision 1.2  2002/08/22 07:04:59  danielss
 *        - Einige Anpassungen an Lecturnity
 *          . Übertragung lässt sich abbrechen,
 *          . Der Status wird über ProgressListener zurückgegeben
 *
 *        Revision 1.1  2002/07/29 13:30:00  danielss
 *        FEATURE [normal]:
 *         - Neue Kompomente im Lecturnity-Baum: Ein FTP-Client.
 *         - Wird für den eCDN-Export zur Benutzung kommen.
 *
...
import imc.lecturnity.util.ProgressListener;
...
public class FTPClient {
   ...
   /**
    * The buffer size to use for transmissions.
    * Defaults to 16kB.
    */
    ...
    *  Revision control id
    */
   private static String cvsId = "$Id: FTPClient.java,v 1.3 2002-08-23 13:25:22 danielss Exp $";
   ...
   /**
    * Set this to true to cancel a transmission
    */
   private boolean cancelRequest_ = false;

   /**
    * The ProgressListener for the transmissions. May be null.
    */
   private ProgressListener progressListener_ = null;
   ...
   public FTPClient(String remoteHost, ProgressListener progressListener)
      throws IOException, FTPException
   {
      ...
      throws IOException, FTPException 
   {
      ...
      // if transmission was cancelled, delete current file
//       if (cancelRequest_)
//          delete(remoteFile);
   }
      ...
      throws IOException, FTPException 
   {
      ...
      // each line
      String line = null;
      while ((line = in.readLine()) != null && !cancelRequest_) {
      ...
      throws IOException, FTPException 
   {
      ...
      int fileSize = (int) (new File(localPath)).length();

      initPut(remoteFile, append);

      if (progressListener_ != null)
      {
         progressListener_.setMaximum(fileSize);
         progressListener_.setValue(0);
      }
      ...
      int count = 0;
      int total = 0;
      while ((count = in.read(buf)) > 0 && !cancelRequest_) {
         out.write(buf, 0, count);
         total += count;
         if (progressListener_ != null)
            progressListener_.setValue(total);
      }
      in.close();
      ...
      throws IOException, FTPException 
   {
      ...
      while ((count = in.read(chunk, 0, chunksize)) >= 0 && !cancelRequest_) {
