...
//	    }
	  ...
	  if (doLMSGetValue( "cmi.entry") == "resume")
		...
		{
		if(g_LMSResumeSupported && g_hasTesting == true && checkSuccessStatus() != null)
			doLMSSetValue( "cmi.score.raw", g_nTotalCurrentlyAchievePoints ) ;
		if(g_hasTesting == true && checkSuccessStatus() != null)
			scutils_updateSuccessStatus();
		scutils_updateCompletionStatus();
		}
	if(g_hasTesting == false)
		window.setInterval("g_sessionTime++", 1000)
	}	
}
...
function delayedSeekTime(msTime)
{
  if (InitImageStyle.visibility == 'hidden')
  {
    ...
    if(g_isAutostartMode == true)
        setTimeout("layout_onGotoTimestamp(g_returnPosition, 'play')", 500);
    else
...
function scutils_setMinMaxScores()
{
    if(g_LMSResumeSupported && g_hasTesting == true)
    {
		doLMSSetValue( "cmi.score.min", g_nTotalMinAchievablePoints ) ;
  ...
  if (g_scormEnabled && g_scormLoaded  )
	  ...
	  for ( var i = 0; i < g_visitedPages.length; i++)
		  if(g_visitedPages[i] == "true")
			  a_visitedCount++;
	  var nNumberOfCopletionPages = g_thumbCount;
	  if(g_isRandomTest)
	  {
	    var nFirstQPage = g_questionEntries[0].nPage;
	    var nLastQPage = g_questionEntries[g_questionEntries.length - 1].nPage;
	    nNumberOfCopletionPages = g_thumbCount -(nLastQPage - nFirstQPage) + 1 + g_nrOfRandomQuestions; 
	  }
	  if(a_visitedCount < nNumberOfCopletionPages)//g_thumbCount)
		{
  ...
  if (g_scormEnabled && g_scormLoaded )
	 {
		...
		if(g_isRandomTest)
		{
		    ...
		    {
		        a_suspendData += g_randomQuestionOrder[k] + g_MinorStrSeparator;
		    }
		    if(g_randomQuestionOrder.length >= 1)
		        a_suspendData += g_randomQuestionOrder[g_randomQuestionOrder.length - 1];
		}
 ...
 if (g_scormEnabled && g_scormLoaded )
	  {
		...
		if (a_suspendDataInformation != null)
		{
				...
				for ( var j = 0; j < a_QuestionData.length; j++)
					{		
						...
						if(a_QuestionValue[2] == "true")
						{
							...
							updateTimerFromQuestionIndex(a_QuestionIndex);
						}							
					}
				}
			...
			// recompose g_randomQuestionOrder data if needed
			if(g_isRandomTest)
			{
			...
			var response = doLMSGetValue("cmi.interactions."+n+".learner_response");
				...
				if(response != null)
					...
					if(type == "choice")
					{
						...
						if(!radioGroup)
							radioGroup = eval("document.TestingForm.Check" + g_questionEntries[qIdx].nPage);
						var j = 0;
						...
						while(j < response.length)
						{
							...
							while(k < count && !(str.length==1 && str.charAt(0)=='-'))
							{
								if(g_targetPointEntries[k].nX == vector[g_Responses.indexOf(str)]) 
								{
								    ...
								    for (i = 0; i < g_interactionEntries.length; ++i)
								    {
								       if(g_interactionEntries[i].strId == g_targetPointEntries[g_questionDragIndex].strObjectId)
									   {
								    ...
								    {
								      currentDndObject.x = parseInt(imgObject.style.left) - layout_getSlidesLeft();
								      currentDndObject.y = parseInt(imgObject.style.top) - layout_getSlidesTop();
								    }
	...
	for (var i = 0; i < g_radioDynamicEntries.length; ++i)
  		{
			...
			if (pageNr == a_pageNumber)
			{
				...
				if(!choiceChecked)
					choiceChecked = eval("document.TestingForm.Check" + a_pageNumber);
