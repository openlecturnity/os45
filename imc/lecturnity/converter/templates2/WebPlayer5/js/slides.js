 ...
 * The "TpEntry" object
 * ------------------------------------------------------------------------ */
function TpEntry(tpTime, x, y, nType, isVisible)
{
  this.tpTime = tpTime;
  this.x = x;
  this.y = y;
...
// Default size of Telepointer and mouse cursor
var g_tpWidth = 30;
var g_tpHeight = 30;

var g_bDoScalePointer = false;

var g_tempRandomJumpValue = -1;
...
function determineDocumentObjects()
{
  ...
  var scaleX = TelepointerImage.width / g_tpWidth;
  var scaleY = TelepointerImage.height / g_tpHeight;
...
function loadSlide(position)
{
  ...
  else
  {
    ...
    for (i = 0; i < g_slideEntries.length; i++)
    {
      ...
      {
        if ((g_slideEntries[i].slideNumber == sldNumber) && (g_slideEntries[i].slideTime <= actualTimestamp))
        {
...
function showTP(idx)
{
  ...
  {
    var scale = g_bDoScalePointer ? g_scale : 1.0;
    
    var bIsTelepointer = (g_tpEntries[idx].nType == 0) ? true : false;

    var tpXOffset = bIsTelepointer ? ((scale - g_scale) * g_tpWidth) : 0;
    var tpX = g_tpEntries[idx].x + g_slidesLeft - tpXOffset;
    var tpY = g_tpEntries[idx].y + g_slidesTop;

    var imgSrc = bIsTelepointer ? "images/telepointer24.gif" : "images/cursor.gif";
    TelepointerImage.src = imgSrc;
    
    TelepointerImage.width = g_tpWidth * scale;
    TelepointerImage.height = g_tpHeight * scale;

    if (g_layoutNavigationDocumentStructure == 2)
    {
      if ((!g_hasVideo) && (!g_hasStillImage))
        tpX = g_tpEntries[idx].x + g_slidesOffset - tpXOffset;
    }
...
function checkForStopmark(msPreviousTime, msActualTime)
{
  ...
  if (msActualTime < lastStopmarkTimestamp)
    lastStopmarkTimestamp = 0;

  var bStopmarkFound = false;
  var msStopmark = -1;

    if ( (g_isRandomTest) &&(g_firstQuestionTimestamp > msPreviousTime) && (g_firstQuestionTimestamp <= msActualTime)
        && (g_tempRandomJumpValue != g_JumpRandomTimestamp))
    {
        if(g_firstQuestionTimestamp == 1)
            g_firstQuestionTimestamp = -1; //question is on first page -> reset g_firstQuestionTimestamp
            
        /* Added g_tempRandomJumpValue in order to avoid multiple calls to
        *  layout_onGotoTimestampR in case the jump is performed to the first
        *  question of the document.
        *  In case there is no question on the first page of the recording
        *  g_firstQuestionTimestamp represents first question page timestamp minus 1 
        */     
        g_tempRandomJumpValue = g_JumpRandomTimestamp; 
       if(g_PlayerType == 0) //WM
            setTimeout("layout_onGotoTimestampR(g_JumpRandomTimestamp, 'play')", 57);
        else
            layout_onGotoTimestampR(g_JumpRandomTimestamp);//(g_questionEntries[g_randomQuestionOrder[0]].nStartMs);
        return true;
    }
  ...
  if (bStopmarkFound && (msStopmark > -1))
  {
    ...
    if ((g_JumpRandomTimestamp > msPreviousTime) && (g_JumpRandomTimestamp <= msActualTime)
        && (msStopmark < g_JumpRandomTimestamp))
    {
    }
    else
    {
        slides_stopAtStopmark(msStopmark);
    }
  }
...
function slides_stopAtStopmark(msStopmark)
{
...
function slides_specialTimestampIsStopmark(msTime)
{
  var bStopmarkFound = false;
  var msStopmark = -1;
