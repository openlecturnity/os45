...
{
   var win = window;
   if((win.opener == null) ||(typeof(window.opener) != "undefined"))
   {
	   win = win.parent;
   }
...
**
*******************************************************************************/
function doLMSGetErrorString( errCode )
{
   // It is permitted to call GetLastError() after Terminate()
...
**
*******************************************************************************/
function doLMSGetDiagnostic( error )
{
   // It is permitted to call GetLastError() after Terminate()
...
function displayErrorInfo( errCode )
{
   if ( _debug )
   {
      var errString = doLMSGetErrorString( errCode );
      var errDiagnostic = doLMSGetDiagnostic( errCode );
