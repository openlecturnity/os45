...
{
  var playerImageObject = getHtmlObject("", "ImageWindowLayer", "PlayerImageWindow");
  var playerImageWindow = getHtmlObject("", "ImageWindowLayer", "PlayerImageWindow", true);
  if (playerImageObject.height <= 1)
    playerImageWindow.visibility = 'hidden';

  var playerStatusBar = getHtmlObject("", "ImageWindowLayer", "PlayerStatusBar", true);
  playerStatusBar.visibility = 'hidden';
...
function controlInit()
{
  ...
  // (Enable/)Disable context menu (RealPlayer 8 only)
  try {
    masterPlayer.SetEnableContextMenu(g_pluginContextMenuEnabled);
  } catch (err) {
  	// do nothing
  }
...
function control_checkUrls()
{
  ...
  var bMediaUrlIsValid = checkUrlIsValid(masterPlayer.GetSource());
  // If 'masterPlayer.GetSource()' returns an empty string, the URL is also not valid
  if (masterPlayer.GetSource().length == 0)
    bHtmlUrlIsValid = bMediaUrlIsValid = false;
  ...
  if (!(bHtmlUrlIsValid && bMediaUrlIsValid))
  {
    ...
    if (!bMediaUrlIsValid)
      strMessage += (masterPlayer.GetSource() + "\n");
...
function control_notifyCurrentTime(valueMs)
{
  ...
  var masterPlayer = control_getMasterPlayer();
  if ( ((actualTimestamp+125) >= g_mediaDurations[0]) && !(control_isPlayerStopped(masterPlayer)) ) // 125ms as buffer
  {
    slides_notifyNewMediaPlayerPosition(actualTimestamp);
    control_stop();
  }
...
function control_invalidateStoppedSeeking()
{
  g_controlIsStoppedSeeking = false;
}
...
function control_play()
{
  ...
  if (!g_pausedPositionValid)
  {
    ...
    setTimeout("control_invalidateStoppedSeeking()", 1250);
  }
  else 
    g_controlIsStoppedSeeking = false;
...
function control_seekTime(valueMs)
{
  ...
  {
    if (g_playerState == 0) // stopped player
      g_controlIsStoppedSeeking = true;
    if (g_playerState == 1) // paused player
      control_setCurrentPosition(masterPlayer, valueMs);
  ...
  if (g_controlPlayerOk)
  {
    ...
    try
    {
      ...
      if (errorCode != 0 && !g_controlErrorIssued)
      {
        ...
        else if (errorCode == PNR_PROXY_NET_CONNECT)
          alert(g_realPnrProxyNetConnect + masterPlayer.GetSource());
		...
		else if (g_isRtspProtocol == true)
		  alert(g_realPnrRtspNotSupported + g_playerUrl);
		else // unknown error
