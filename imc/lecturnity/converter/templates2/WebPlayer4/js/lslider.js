...
function LSlider(theFieldnamePrefix)
{
  this._objectId;
  this.fieldName;
  ...
  {
    this._id = LObjects.length;
    LObjects[this._id] = this;
    this._objectId = "LSlider_" + this._id;
  ...
  {
    // IE8: call of ".innerHtml" (see below) seems to need a delay
    var currentTime = new Date().getTime();
    // Delay of 0.25 sec (250 msec) reached?
    if (currentTime < (g_lsliderCurrentTime + 250))
      return;
    g_lsliderCurrentTime = currentTime;
    ...
    if ((this.arrowAmount > 0) && this._arrowIconLeftSrc)
    {
      ...
      out[outI++] = '<a href="javascript:void(false);" onClick="LObjects[' + this._id + '].onChangeByArrow(false); return false;">';
    ...
    if (this.colorbar)
    {
      out[outI++] = '<div id="' + this._colorbarId + '" onClick="LObjects[' + this._id + '].onChangeByClick(event);"';
      ...
      out[outI++] = '">';
      out[outI++] = '<img src="spacer.gif" width="1" height="5"></div>';
    }
    out[outI++] = '<div id="' + this._slideBarId + '" onClick="LObjects[' + this._id + '].onChangeByClick(event);" style="position:absolute; left:' + localOffset + '; top:0; width:' + this.width + '; height: ' + divHeight + '; clip:rect(0 ' + divWidth + '  ' + divHeight + ' 0);';
    if (this.bgColor)
    {
    ...
    if ((this.arrowAmount > 0) && this._arrowIconRightSrc)
    {
      ...
      out[outI++] = '<a href="javascript:void(false);" onClick="LObjects[' + this._id + '].onChangeByArrow(true); return false;">';
    ...
    else if (this.useInputField == 2)
    {
      ...
      out[outI++] = ' onKeyUp="LObjects[' + this._id + '].onChangeByInput(this.value, false);" onBlur="LObjects[' + this._id + '].onChangeByInput(this.value, true);">';
    }
    else if (this.useInputField == 3)
    {
      ...
      out[outI++] = ' onKeyUp="LObjects[' + this._id + '].onChangeByInput(this.value, false);" onBlur="var _bss = LObjects[' + this._id + ']; _bss.onChangeByInput(this.value, true); _bss.textboxEdit(false)">';
      out[outI++] = '<span' + styleValueTextClass + ' style="" id="' + this._valueTextId + '" ';
      if (this.inputTextFieldEvent == 'click')
      {
        out[outI++] = 'onClick="LObjects[' + this._id + '].textboxEdit(true);"';
      }
      else
      {
        out[outI++] = 'onMouseOver="LObjects[' + this._id + '].textboxEdit(true);"';
      }
    ...
    if (this.valueDefault > this.minVal)
    {
      ...
      var myPercent = parseFloat(this.valueDefault-this.minVal) * 100.0 / parseFloat(hundertPercent);
      this._currentRelSliderPosX += (myPercent * this._slideWidth / 100.0);
  ...
  this.setDisabled = function(b)
  {
    ...
    var t = new Array(this._containerId, 
                      this._arrowLeftContainerId, 
                      this._arrowRightContainerId, 
                      this._valueFieldId, 
                      this._valueTextId,
  ...
  this._updateColorbar = function(newPos)
  {
    ...
    {
      var maxPos = this._posSlideStart + this.width - this._sliderImgWidth;
      if (newPos > maxPos)
        newPos = maxPos;
  ...
  this._getNewLocationFromCursor = function()
  {
    ...
    switch(this.direction)
    {
      case 0:
        ...
        break;
      case 1:
        if(this.ctrl.pageY>this.ctrl.maxY)
          y=this.ctrl.maxY;
        else if(this.ctrl.pageY<this.ctrl.minY)
          y=this.ctrl.minY;
        else
        {
  ...
  this.updateHandle = function(newPos)
  {
    ...
    switch(this.direction)
    {
      case 0:
        ...
        break;
      case 1:
        moveDivTo(this.ctrl.div, getDivLeft(this.ctrl.div), newPos);
        break;
    }
  }
