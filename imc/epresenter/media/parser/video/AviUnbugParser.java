...
import imc.epresenter.media.protocol.epf.EpfSourceStream;
import imc.lecturnity.util.NativeUtils;

public class AviUnbugParser extends BasicPullParser {
   //static { System.out.println("The one and only AviUnbugParser!"); }
   ...
   //
   public static AviUnbugParser register(String name)
   {
      instances_.put(name, new WeakReference(lastParser_));
      return lastParser_;
   }

   public static void cleanUp(String name)
   {
      WeakReference ref = (WeakReference)instances_.get(name);
      if (ref != null && ref.get() != null)
      {
         AviUnbugParser p = (AviUnbugParser)ref.get();

         p.tracks = null;
         p.trakList = null;
         // close() not needed anymore; now done in epf/DataSource
         //if (p.stream != null && p.stream instanceof EpfSourceStream)
         //   ((EpfSourceStream)p.stream).close();
         // p.seekableStream is the same as p.stream
         
         p.stream = null;
         p.seekableStream = null;
         p.cacheStream = null;

         instances_.put(name, null);
      }
   }
      ...
      try {
         ...
         if (trackType.equals(VIDEO)) {
            ...
            if (intArray[0] > 32) {
               ...
               // Divx 5.0 is not recognized as handler for Divx 5.0 videos
               // reason: JMF finds handlers only by looking for an
               // appropriate fourcc handler; Divx 5.0 has itself registered
               // as Divx 4.0 handler (DIVX instead of DX50)
               // 
               // this is a work around; if and only if Divx 5.0 is installed 
               //
               // BUGFIX 2251: extended this workaround for all installed DivX 
               // versions equal or later than 5.
               if (video.compressor.equals("DX50"))
               {
                  String divxDesc = 
                     NativeUtils.getRegistryValue(
                        "HKLM", 
                        "SYSTEM\\CurrentControlSet\\Control\\MediaResources\\icm\\vidc.DIVX",
                        "Description");

                  if (divxDesc != null && divxDesc.length() > 6)
                  {
                     // check for a version equal or higher to "5.x.x"
                     int nIdxPoint = divxDesc.indexOf('.');
                     int nIdxSpace = divxDesc.indexOf(' ');
                     if (nIdxPoint > -1 && nIdxSpace > -1 && nIdxSpace < nIdxPoint-1)
                     {
                        int nVersion = -1;
                        try
                        {
                           nVersion = Integer.parseInt(divxDesc.substring(nIdxSpace+1, nIdxPoint));
                        }
   ...
   private void parseIDX1(int length) throws BadHeaderException {
      try {
         ...
         for (int i = 0; i < numIndices; i++) {
            ...
            chunkInfo[index].id = (String)idMap.get(id);
   ...
   public Time findPreviousKeyFrame(Time where)
   {
      if ( (keyFrameTrack != -1) && (tracks[keyFrameTrack].isEnabled()) )
      {
         ...
         //return where;
         return track.mapFrameToTime(keyframeNum);
      }
      else
      {
         return null;
   ...
   private abstract class MediaTrack implements Track {
      ...
      private AviUnbugParser parser = AviUnbugParser.this;
   ...
   private class VideoTrack extends MediaTrack  {
      ...
      long getTimeStamp() {
         ...
         return (1000L * useChunkNumber * usecPerFrame);
      }
