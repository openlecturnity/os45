...
/*   m_buttonImageList.Create(cx, cy, ILC_COLOR24, 1, nImgs );
   ...
   bitmap.DeleteObject();
   */
   SetImageList(bitmapID, cx, cy, nImgs);
...
void CImageButton::SetImageList(UINT nBitmapID, int cx, int cy, int nImgs, bool refresh)
{
   if (m_buttonImageList.m_hImageList)
      m_buttonImageList.DeleteImageList();
   m_buttonImageList.Create(cx, cy, ILC_COLOR24, 1, nImgs);
   CBitmap bitmap;
...
void CImageButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
   ...
   if (!m_font.m_hObject)
   {
      ...
      m_font.CreatePointFont(fontSize, _T("Arial"), pDC);
   }
   if (m_font.m_hObject)
   {
