...
Value::Value(const CString &value)
    : type_(stringValue)
    , allocated_(true)
    , comments_(0)
# ifdef JSON_VALUE_USE_INTERNAL_MAP
...
#endif
{
    char *ansiText = LString::WideChar2Ansi(value);
    std::string newVal = ansiText;
    SAFE_DELETE(ansiText);
    value_.string_ = valueAllocator()->duplicateStringValue(newVal.c_str(), 
                                                            (unsigned int)newVal.length());
}
...
Value &Value::operator[](const CString &key) {
    char *ansiText = LString::WideChar2Ansi(key);
    std::string newVal = ansiText;
    SAFE_DELETE(ansiText);
    return (*this)[ newVal.c_str() ];
}

const Value &Value::operator[](const CString &key) const {
    char *ansiText = LString::WideChar2Ansi(key);
    std::string newVal = ansiText;
    SAFE_DELETE(ansiText);
    return (*this)[ newVal.c_str() ];
