 ...
 * REVIEW UK
 *
 * Please also see CControlLineStyleSelector for a clean layout.
 *
 * Remove CLineWidth. Doubles can be saved in a simple array.
 * With a CArray there would also be a GetSize().
 */
...
void CControlLineWidthSelector::Draw(CDC* pDC)
{
	...
	for (int i = 0; i < nCount; i++)
	{
		...
		if (i == m_nSelected || lineWidths[i].dLineWidth == m_dLineWidth)
		{
         ...
         GetPaintManager()->DrawRectangle(pDC, &rcButton, i == m_nSelected, bPressed, m_bEnabled, lineWidths[i].dLineWidth == m_dLineWidth, FALSE,GetParent()->GetType(), GetParent()->GetPosition());
		}
			
		Gdiplus::Graphics gdipGraphics(pDC->m_hDC);
		Gdiplus::Pen pen(Gdiplus::Color(255, 0, 0, 0), (float)lineWidths[i].dLineWidth);
