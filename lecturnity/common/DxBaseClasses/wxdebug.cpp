...
void WINAPI DbgBreakPoint(const TCHAR *pFileName,INT iLine,const TCHAR* szFormatString,...)
{
    ...
    int nReturnValue = _vsntprintf_s( szBreakPointMessage, MAX_BREAK_POINT_MESSAGE_SIZE, 
        MAX_CHARS_IN_BREAK_POINT_MESSAGE, szFormatString, va );
...
void WINAPI DbgLogInfo(DWORD Type,DWORD Level,const TCHAR *pFormat,...)
{
    ...
    int iAddLength = lstrlen(szInfo);
    _vstprintf_s(szInfo + iAddLength, 2000 - iAddLength, pFormat, va);
    lstrcat(szInfo, TEXT("\r\n"));
    DbgOutString(szInfo);
