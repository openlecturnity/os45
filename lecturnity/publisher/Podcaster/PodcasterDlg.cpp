// PodcasterDlg.cpp : implementation file
//
...
UINT __cdecl SessionThread(LPVOID pParam)
{
   ...
   CPodcasterDlg *dlg = (CPodcasterDlg *)pParam;
...
/////////////////////////////////////////////////////////////////////////////
// CPodcasterDlg dialog

CPodcasterDlg::CPodcasterDlg(CWnd* pParent /*=NULL*/)
   : CDialog(CPodcasterDlg::IDD, pParent)
{
   //{{AFX_DATA_INIT(CPodcasterDlg)
	m_csFrom = _T("");
	m_csTo = _T("");
	//}}AFX_DATA_INIT
...
void CPodcasterDlg::DoDataExchange(CDataExchange* pDX)
{
   CDialog::DoDataExchange(pDX);
   //{{AFX_DATA_MAP(CPodcasterDlg)
...
BEGIN_MESSAGE_MAP(CPodcasterDlg, CDialog)
   //{{AFX_MSG_MAP(CPodcasterDlg)
   ON_WM_SYSCOMMAND()
   ON_WM_PAINT()
   ON_WM_QUERYDRAGICON()
	ON_WM_CLOSE()
...
/////////////////////////////////////////////////////////////////////////////
// CPodcasterDlg message handlers

BOOL CPodcasterDlg::OnInitDialog()
{
...
/*
   CString csCaption;
   csCaption.LoadString(IDD_PODCASTER_DIALOG);
...
void CPodcasterDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
HCURSOR CPodcasterDlg::OnQueryDragIcon()
{
   return (HCURSOR) m_hIcon;
}

CString CPodcasterDlg::GetFileName(CString csFilePath)
{
   CString csFileName;
...
CString CPodcasterDlg::GetHostName()
{
   return m_csServer;
}

BOOL CPodcasterDlg::CompareFiles(CString srcFileName, CString dstFileName)
{
   BOOL bResult = FALSE;
   BYTE btSrc, btDst;
...
BOOL CPodcasterDlg::IsMediaUrlOk(CString &csURL)
{
   BOOL bResult;
   DWORD dwServiceType;
...
void CPodcasterDlg::OnCancel() 
{
   ...
   if(m_Thread != NULL)
   {
      ...
      // if user press cancel delete mp4 file
      if(m_iService == 0)
      {
         // ftp
         if(bDoDelete)
         {
            ...
            {
               CString csMp4 = m_csRemoteFileName; //GetFileName(csMediaFile);
               ftp.Delete(csMp4, m_csMediaDirectory);
               ftp.Close();
            }
         }
      }
      else
      {
         ...
         if(bDoDelete)
         {
            CString csMp4 = m_csMediaDirectory + m_csRemoteFileName; //GetFileName(csMediaFile);
            SshDelete(csMp4, FALSE);
         }
      }
   }
   CDialog::OnCancel();
...
// new version
/////////////////////////////////////
CString CPodcasterDlg::GetActiveChannel()
{
   CRegistry reg;
   int iActive = 0;
   HKEY hKey = NULL;
...
DWORD CPodcasterDlg::GetFileLength(CString csFile)
{
   DWORD dwLength = 0;
...
BOOL CPodcasterDlg::AddItem(LPCTSTR srcFileName, LPCTSTR dstFileName)
{
...
void CPodcasterDlg::LoadFromRegistry(CString pszKey)
{
...
BOOL CPodcasterDlg::AskForPassword()
{
   BOOL bResult = FALSE;
   CPasswordDlg dlgPsw;
...
BOOL CPodcasterDlg::DoSession()
{
   BOOL bResult = FALSE;
   
   // 1. get active channel
   csActiveChannel = GetActiveChannel();
   if(csActiveChannel.IsEmpty())
   {
      CString csMessage;
      csMessage.LoadString(IDM_ACTIVE);
      AfxMessageBox(csMessage, MB_ICONSTOP);
     ::PostMessage(m_hWnd, WM_CLOSE, 0, 0);
      return FALSE;
   }
   
   // 2. load params for active channel from registry
   ...
   while(TRUE)
   {
      mp4.m_csFileName = m_csRemoteFileName;
      mp4.m_csUrl = m_csMediaFileUrl;
      int iResult = mp4.RemoteFileExists();
      ...
      if(iResult == 1)
      {
         ...
         // modified
         m_csRemoteFileName = mp4.m_csFileName;
         break;
      }
   } 
   ...
   // 3. try to insert new item
   CString csFile = pApp->csTmpDirectory + _T("\\tmp.xml");
   if(!AddItem(m_csChannel, csFile))
   {
     ::PostMessage(m_hWnd, WM_CLOSE, 0, 0);
      return FALSE;
   }
...
BOOL CPodcasterDlg::DoFtpSession()
{
   BOOL bResult = FALSE;
   ...
   while(TRUE)
   {
      ...
      bDoDelete = TRUE;
      bResult = ftp.Upload(csMediaFile, csMp4, m_csMediaDirectory, m_prgProgress);
      bDoDelete = FALSE;
...
int CPodcasterDlg::CallSsh(int argc, WCHAR** argv, HWND handle, int iDelete = 0)
{
...
int CPodcasterDlg::CallSsh(int argc, char** argv, HWND handle, int iDelete = 0)
{
   int iResult = 1;
   if(hModule == NULL)
   {
...
LRESULT CPodcasterDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
...
BOOL CPodcasterDlg::SshUpload(CString csSrcFileName, CString csDstFileName, BOOL bQuit = FALSE)
{
   BOOL bResult = FALSE;
   iArgc = 0;
...
BOOL CPodcasterDlg::SshDownload(CString csSrcFileName, CString csDstFileName, BOOL bQuit = FALSE)
{
   BOOL bResult = FALSE;
   iArgc = 0;
...
BOOL CPodcasterDlg::SshDelete(CString csSrcFileName, BOOL bQuit)
{
   BOOL bResult = FALSE;
...
BOOL CPodcasterDlg::DoSshSession()
{
   BOOL bResult = FALSE;
   ...
   bDoDelete = TRUE;
   bResult = SshUpload(csMediaFile, csMp4, FALSE);
   bDoDelete = FALSE;
...
void CPodcasterDlg::SetInfoText(CString csFrom, CString csTo)
{
