...
CString CFtp::GetInetError(DWORD dwErrorNo)
{
   ...
   { /* got module */
      ...
      { /* format succeeded */
		 ...
		 AfxMessageBox(csErrorMessage, MB_OK + MB_ICONSTOP);
...
BOOL CFtp::Upload(CString csSrcFileName, CString csDstFileName, CString csDirectory, CProgressCtrl &m_prgProgress) 
{
   ...
   if(bResult)
   {
      ...
      {
		 CString csTemp;
		 csTemp.Format(_T("%s%s"), m_csCurrentDirectory, csDirectory);
		 //bResult = m_FtpConnection->SetCurrentDirectory(m_csCurrentDirectory + csDirectory);
		 bResult = m_FtpConnection->SetCurrentDirectory(csTemp);
         if(bResult)
         {
...
int CFtp::ErrorMessage(UINT uId)
{
   ...
   MessageBox(NULL, csMessage, _T("SlideStarUpload"), MB_OK + MB_ICONSTOP);
   return 0;
}
