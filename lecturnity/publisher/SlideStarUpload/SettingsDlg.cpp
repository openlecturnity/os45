   ...
   : CDialog(CSettingsDlg::IDD, pParent)
{
   ...
   m_bSavePassword = FALSE;
	...
	m_bPrivateKey = FALSE;
	//}}AFX_DATA_INIT
   m_hIcon = AfxGetApp()->LoadIcon(IDI_SLIDESTARUPLOAD);
}
...
void CSettingsDlg::DoDataExchange(CDataExchange* pDX)
{
   ...
   DDX_Text(pDX, IDE_ZIPDIR, m_csMediaDirectory);
...
BEGIN_MESSAGE_MAP(CSettingsDlg, CDialog)
   ...
   ON_WM_PAINT()
	...
	ON_EN_KILLFOCUS(IDE_ZIPDIR, OnChangeZipdir)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CSettingsDlg::LoadFromRegistry(CString pszKey)
{
   ...
   if(reg.Open(pszKey) == ERROR_SUCCESS)
   {
      ...
      m_iIPVersion   = reg.GetProfileInt(_T("IPVersion"), 0);
...
void CSettingsDlg::OnChangeZipdir() 
{
   CWnd *pMediaDirectory = GetDlgItem(IDE_ZIPDIR);
