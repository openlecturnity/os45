 ...
 * to licensing issues.)
 */
static void marius_delete(int argc, char *argv[]);
...
static void sourceUploader(char *src, double *dProgress, boolean *pbIsCancel);
...
void win32_message(char *str, int kind)
{
   if(kind == 0)
   {
      //MessageBox(NULL, str, "SSH Error", MB_OK);
      //SendMessage(hWnd, WM_SSH_ERROR, (WPARAM)strlen(str) ,(LPARAM)str);
   }
   else
   {
      //SendMessage(hWnd, WM_SSH_VERBOSE, (WPARAM)strlen(str) ,(LPARAM)str);
   }
}

void win32_status(char *str)
{
   win32_message(str, 1);
}

void win32_errmsg(char *str)
{
   win32_message(str, 0);
}

void win32_progress(int total, int done)
{
   SendMessage(hWnd, WM_SSH_STATUS, total, done);
}

void win32_reqexit(int code)
{
	//exit(1);
   ExitThread(code);
   //SendMessage(hWnd, WM_SSH_REQEXIT, code, 0);
}
...
{
   // marius
   win32_progress(size.lo, done.lo);
   return;
}
...
static void sourceUploader(char *src, double *dProgress, boolean *pbIsCancel)
{
   ...
   for (i = uint64_make(0,0);
      (uint64_compare(i,size) < 0 && !*pbIsCancel);
      i = uint64_add32(i,4096)) 
   {
         char transbuf[4096];
         int j, k = 4096;
         ...
         *dProgress = *dProgress + (k * 1.0) / uint64_to_double(size) * 0.95;
   }
   close_rfile(f);
 ...
 */
static void toremote(int argc, char *argv[], double *dProgress, boolean *pbIsCancel)
{
	...
	*host++ = '\0';
	...
	if (wc_type == WCTYPE_NONEXISTENT) {
		*ErrorCode = 7;
	    ...
	    sourceUploader(src, dProgress, pbIsCancel);
	    continue;
	} else {
	    WildcardMatcher *wc;
	    char *filename;
...
int pscp_main(int argc, char *argv[], HWND hWndParrent, int mar_delete, double *dProgress, boolean *pbIsCancel, boolean *pbIsActive, int *piError, int iCheck, int* piExist)
{
    int i;
	piExists = piExist;
	ErrorCode = piError;
    // marius
   //MessageBox(NULL, "in main", "SSH Error", MB_OK | MB_TOPMOST);
    marius_init();
    hWnd = hWndParrent;
   ...
   // marius 
	...
	ret = cmdline_process_param(argv[i], i+1<argc?argv[i+1]:NULL, 0, &cfg);
	if (ret == -2) {
		*ErrorCode = 105;
   ...
   {
	    toremote(argc, argv, dProgress, pbIsCancel);
   }
	else
   {
       if(mar_delete == 1)
       {
         marius_delete(argc, argv);
       }
	   else if (iCheck == 1)
	   {
		   check(argc, argv);
...
static void marius_delete(int argc, char *argv[])
{
	...
	{
		*ErrorCode = 21;
	   bump("delete file not supported");
	}
    *src++ = '\0';
    if (*src == '\0')
	src = ".";
	...
	*host++ = '\0';
    ...
    //cmd = dupprintf("unlink %s", src);
    cmd = dupprintf("rm -r %s", src);
    do_cmd(host, user, cmd);
    sfree(cmd);
    if (using_sftp) 
    {
       ...
       sftp_register(req = fxp_remove_send(src));
       rreq = sftp_find_request(pktin = sftp_recv());
       assert(rreq == req);
       fxp_remove_recv(pktin, rreq);
    }
}
...
//         user = NULL;
//   }
//   //check is src(targ) exists on server
//
//   //set *piExists = 0 (not exists), 1 (exists, not dir), 2 (exists, dir, empty), 3 (exists, dir, not empty)
//
//
//   //src = "/apache/htdocs/tmp/test";
//   cmd = dupprintf("ls -d %s", src);
//   do_cmd(host, user, cmd);
//   sfree(cmd);
//   
//   while (ssh_scp_recv((unsigned char *) &c, 1) > 0)
...
//      *piExists = 1;
//
//   if(*piExists == 3 && !isFile)
//   {
//      cmd = dupprintf("ls %s", src);
//      do_cmd(host, user, cmd);
//      sfree(cmd);
//
//      if(ssh_scp_recv((unsigned char *) &c, 1) == 0)
//         *piExists = 2;
//   }    
//   pendlen = 0;
//   return;
...
static void check (int argc, char* argv[])
{
	...
	*host++ = '\0';
    ...
    if (using_sftp) 
    {
       ...
       sftp_register(req = fxp_stat_send(src));
       rreq = sftp_find_request(pktin = sftp_recv());
       assert(rreq == req);
       ret = fxp_stat_recv(pktin, rreq, &attrs);

       //check if src exists on server
       if (ret || (attrs.flags & SSH_FILEXFER_ATTR_PERMISSIONS))
       {
          *piExists = 3;
       }
       ...
       //if src is a directory, let's see if it's empty
       if(*piExists == 3)
       {
          sftp_register(req = fxp_opendir_send(src));
          rreq = sftp_find_request(pktin = sftp_recv());
          assert(rreq == req);
          dirh = fxp_opendir_recv(pktin, rreq);
