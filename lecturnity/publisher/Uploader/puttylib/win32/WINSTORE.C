...
#define SHGFP_TYPE_CURRENT 0	//marius
...
static HANDLE access_random_seed(int action)
{
   ...
   */
   ...
   if (RegOpenKeyA(HKEY_CURRENT_USER, PUTTY_REG_POS, &rkey) ==
      ERROR_SUCCESS) {
         int ret = RegQueryValueExA(rkey, "RandSeedFile",
   ...
   */
   if (!tried_shgetfolderpath) {
      ...
      * so stuff that. */
      shell32_module = LoadLibraryA("SHELL32.DLL");
      if (shell32_module) {
   ...
   */
   {
      ...
      len =
         GetEnvironmentVariableA("HOMEDRIVE", seedpath,
         sizeof(seedpath));
      ret =
         GetEnvironmentVariableA("HOMEPATH", seedpath + len,
         sizeof(seedpath) - len);
      if (ret != 0) {
         strcat(seedpath, "\\PUTTY.RND");
         if (try_random_seed(seedpath, action, &rethandle))
   ...
   */
   GetWindowsDirectoryA(seedpath, sizeof(seedpath));
   strcat(seedpath, "\\PUTTY.RND");
   if (try_random_seed(seedpath, action, &rethandle))
      return rethandle;
