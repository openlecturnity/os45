...
// marius
char *winsock_error_string2(int error);
...
void sk_init(void)
{
   ...
   if (!winsock_module) {
      winsock_module = LoadLibraryA("WSOCK32.DLL");
   }
   if (!winsock_module)
   ...
   if (GetProcAddress(winsock_module, "getaddrinfo") != NULL) {
      ...
      wship6_module = LoadLibraryA("wship6.dll");
      if (wship6_module) {
#ifdef NET_SETUP_DIAGNOSTICS
...
Socket sk_register(void *sock, Plug plug)
{
   ...
   */
   ...
   if (ret->s == INVALID_SOCKET) {
      err = p_WSAGetLastError();
      ret->error = winsock_error_string2(err);
      return (Socket) ret;
   }
...
static DWORD try_connect(Actual_Socket sock)
{
   ...
   * If the stack supports IPv6 it will also allow IPv4 connections. */
   ...
   if (s == INVALID_SOCKET) {
      err = p_WSAGetLastError();
      sock->error = winsock_error_string2(err);
      goto ret;
   }
      ...
      if (nsent <= 0) {
         ...
         if ((err < WSABASEERR && nsent < 0) || err == WSAEWOULDBLOCK) {
            ...
            * that the frontend handle is unnecessary. */
            logevent(NULL, winsock_error_string2(err));
            fatalbox("%s", winsock_error_string2(err));
         }
      } else {
         if (s->sending_oob) {
            if (nsent < len) {
      ...
      */
      if (s->addr) {
         plug_log(s->plug, 1, s->addr, s->port,
            winsock_error_string2(err), err);
         while (s->addr && sk_nextaddr(s->addr)) {
            err = try_connect(s);
         }
      }
      if (err != 0)
         return plug_closing(s->plug, winsock_error_string2(err), err, 0);
      else
         return 1;
   }
...
case FD_READ:
   ...
   */
   ...
   if (ret < 0) {
      return plug_closing(s->plug, winsock_error_string2(err), err,
         0);
   } else if (0 == ret) {
   ...
   */
   ...
   if (ret <= 0) {
      char *str = (ret == 0 ? "Internal networking trouble" :
         winsock_error_string2(p_WSAGetLastError()));
      /* We're inside the Windows frontend here, so we know
      * that the frontend handle is unnecessary. */
      logevent(NULL, str);
      fatalbox("%s", str);
...
case FD_CLOSE:
   ...
   do {
      ...
      if (ret < 0) {
         ...
         if (err == WSAEWOULDBLOCK)
            break;
         return plug_closing(s->plug, winsock_error_string2(err),
            err, 0);
      } else {
         if (ret)
   ...
   do {
      for (i = 0; (s = index234(sktree, i)) != NULL; i++) {
         if (s->pending_error) {
            ...
            */
            plug_closing(s->plug,
               winsock_error_string2(s->pending_error),
               s->pending_error, 0);
            break;
         }
      }
...
char *winsock_error_string2(int error)
{
   char *err = winsock_error_string(error);
   //   win32_errmsg2(err);
   return err;
}
