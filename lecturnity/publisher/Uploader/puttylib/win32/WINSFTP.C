                          ...
                          unsigned long *mtime, unsigned long *atime)
{
   ...
   h = CreateFileA(name, GENERIC_READ, FILE_SHARE_READ, NULL,
      OPEN_EXISTING, 0, 0);
...
WFile *open_new_file(char *name)
{
   ...
   h = CreateFileA(name, GENERIC_WRITE, 0, NULL,
...
DirHandle *open_directory(char *name)
{
   ...
   h = FindFirstFileA(findfile, &fdat);
   if (h == INVALID_HANDLE_VALUE)
      return NULL;
   sfree(findfile);
...
char *read_filename(DirHandle *dir)
{
   do {
      ...
      if (!dir->name) {
         WIN32_FIND_DATAA fdat;
         int ok = FindNextFileA(dir->h, &fdat);
         if (!ok)
            return NULL;
         else
...
int test_wildcard(char *name, int cmdline)
{
   ...
   if (GetFileAttributesA(name) != (DWORD)-1)
      return WCTYPE_FILENAME;

   /* Otherwise see if a wildcard match finds anything. */
   fh = FindFirstFileA(name, &fdat);
...
*/
/*
int win_sftp(int argc, char *argv[])
{
int ret;
