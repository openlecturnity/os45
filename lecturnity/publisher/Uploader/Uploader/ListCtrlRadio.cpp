...
void CListCtrlRadio::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) {
    ...
    szBuff[0] = 0;
    GetItemText(nItem, 1, szBuff, sizeof(szBuff));

    this->GetSubItemRect(nItem, 1, LVIR_LABEL, rcLabel);
    pDC->DrawText(szBuff, -1, rcLabel, dwFlags);


    // draw third column - state icon
    lvi.mask = LVIF_STATE;
    lvi.iItem = nItem;
    lvi.iSubItem = 2;
    lvi.pszText = szBuff;
    ...
    GetSubItemRect(nItem, 2, LVIR_BOUNDS, rcLabel);
    // clear subitem
    CBrush cbr(RGB(255, 255, 255));
    pDC->FillRect(rcLabel, &cbr);
