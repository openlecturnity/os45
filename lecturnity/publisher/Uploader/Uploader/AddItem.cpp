...
BOOL CAddItem::MoveOnFront(LPCTSTR tstrXmlFileOut) {
    ...
    if (dst.Open(dstFileName,  CFile::modeCreate | CFile::modeWrite, NULL)) { 
        // first chunk
        dwChunk = (DWORD)(first_item - buffer) ;
        dst.Write((void *)buffer, dwChunk);
        dwTotal += dwChunk;

        // last item
        dwChunk = (DWORD)(endof_last_item - last_item);
        dst.Write((void *)last_item, dwChunk);
        dwTotal += dwChunk;

        dst.Write((LPCSTR)szSeparator, (UINT)strlen(szSeparator));

        // body
        dwChunk = (DWORD)(last_item - first_item);
