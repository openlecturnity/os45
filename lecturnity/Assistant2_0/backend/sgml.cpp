...
ASSISTANT::SGMLElement::SGMLElement()
{
...
void ASSISTANT::SGMLElement::ParseString(CString &strn)
{
   Attribute
      *att;
   int 
...
LPTSTR ASSISTANT::SGMLElement::GetAttribute(LPCSTR att) 
{
   for (int i = 0; i < attributes.GetSize(); ++i)
   {
...
int ASSISTANT::SGMLElement::verify(CString &strn)
{
  int i = 0;
  if (strn[i] == _T('/'))
     ++i;
  ...
  {
     CString csMessage;
     csMessage.Format(IDS_ERROR_PARSE_DOCUMENT, tmp, Name);
...
ASSISTANT::SGMLTree::SGMLTree(CString &sfilename)
{
  root = NULL;
...
ASSISTANT::SGMLTree::~SGMLTree()
{
   ASSISTANT::SGMLElement
      *tmp;
...
ASSISTANT::SGMLElement *ASSISTANT::SGMLTree::EvaluateTag(ASSISTANT::SGMLElement *hilf, CString &str)
{
   CString
      zw;
   ...
   if (hilf) 
   {
      if (zw[0] == _T('/')) 
      {
         if (hilf->closed) 
         {
            ...
            {
               CString csMessage;
               csMessage.LoadString(IDS_ERROR_PARSE_PARENT);
               MessageBox(NULL, csMessage, NULL, MB_OK | MB_TOPMOST);
            }
         }
         hilf->verify(zw);
   ...
   {
      hilf = new ASSISTANT::SGMLElement();
      root = hilf;
      hilf->ParseString(zw);
   }
...
void ASSISTANT::SGMLTree::CopyToTchar(BYTE *buffer, UINT nBufLen, TCHAR *tszString, UINT nTcharLen)
{
#ifdef _UNICODE
...
int ASSISTANT::SGMLTree::ReadTcharFromFile(FILE *fp, TCHAR &tcChar)
{
   size_t bytesRead = 0;
   if (m_iFileType == FILE_TYPE_ASCII)
   {
...
int ASSISTANT::SGMLTree::Buildtree(int &pages, CProgressDialogA *pProgress, bool bOnlyInformation)
{
   SGMLElement
      *hilf,
      *hilf2;
   ...
   { 
	  ...
	  HWND hw=pProgress->GetSafeOwner()->GetSafeHwnd();
	  MessageBox(hw, csMessage, NULL, MB_OK | MB_TOPMOST);
      return -1;
   }
   ...
   while (ReadTcharFromFile(datei, tcChar)) 
   {
      ...
      else if ((tcChar == _T('>')) && (ident==0)) 
      {
         ...
         if (bOnlyInformation && hilf->closed && hilf->Name == _T("FOLDER")) pages++;
         if (bOnlyInformation && pages > 0)
             break;
      }
      else if ((tcChar == _T('"')) && (ident==1)) 
      {
